...
add_action( 'rest_api_init', function () {
  ...
  register_rest_route( 'schoolistit/v2', '/get_nearby_schools', array(
    'methods' => 'GET, POST',
    'callback' => __NAMESPACE__.'\\get_nearby_schools',
  ) );

  register_rest_route( 'schoolistit/v2', '/ggl_translate', array(
    'methods' => 'GET, POST',
    'callback' => __NAMESPACE__.'\\ggl_translate',
  ) );
...
/**
 * GGL_Translate
 * GET: return languages available
 * POST: send english return translation
 */
function ggl_translate(\WP_REST_Request $request){
...
function ggl_languages(){

}

/**
 * dont get confused by this but Im about to serve up my damn token via wordpress...were just doing this for testing
 * 
 */
function get_nearby_schools(\WP_REST_Request $request){
  $ggl_response = array();
  ...
  foreach($inputs as $input){
    $key = get_option('google_places_key');
    $url = "https://maps.googleapis.com/maps/api/place/findplacefromtext/json?";
    $args = array(
      "input" => urlencode($input),
      "inputtype" => "textquery",
      "fields" => "photos,formatted_address,name",
      'locationbias' => 'circle:16093@'.$request_params['lat'].','.$request_params['lng'],
      'key'=>$key
    );
    foreach($args as $key=>$value){
      $url .= '&'.$key.'='.$value;
    }
    $search = json_decode(wp_remote_retrieve_body(wp_remote_get($url)));
    foreach($search->candidates as $school){
      $ggl_response[] = $school;
    }
  }
  $ggl_response = array_unique($ggl_response, SORT_REGULAR);
  return $ggl_response;
}
...
function post_content(\WP_REST_Request $request){
  ...
  update_post_meta($post_id, 'link_external', $params['linkExternal']); //store the draft.js raw blocks into metadata for retrieval later.
  $postarr = array(
    'ID'=> $post_id,
    ...
    if($auth_response['authenticated'] === true){
      ...
      $post_array = array(
        ...
        'meta_input' => array(
            ...
            'link_external' => sanitize_text_field($params['link_external']),
      ...
      update_post_meta($post_id, 'link_external', sanitize_text_field($params['linkExternal']));
      //handle the terms
  ...
  function edit_assignment($params){
    ...
    if(in_array('delete_post', $changed_fields)){
      ...
      update_post_meta($params['post_id'], 'link_external', sanitize_text_field($params['linkExternal']));
